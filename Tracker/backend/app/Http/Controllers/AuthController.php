...
class AuthController extends Controller
{
   ...
   public function addRecord(Request $request)
    ...
    public function updateTimeNotification(Request $request)
    {
        try {
            $validated = $request->validate([
                'record_id' => 'required|exists:add_record,id',
            ...
            $record = AddRecord::findOrFail($validated['record_id']);

            // Determine notification status and color
            $notificationStatus = 'In Progress';
            $statusColor = 'bg-red-600 text-red-600';

            if ($validated['vm_status'] === 'Completed' && 
                ($record->document_type !== 'Ordinance' || $validated['cm_status'] === 'Completed')) {
                $notificationStatus = 'Completed';
                $statusColor = 'bg-blue-600 text-blue-600';
            } elseif ($validated['vm_status'] === 'Not Started' && 
                    (!isset($validated['cm_status']) || $validated['cm_status'] === 'Not Started')) {
                return response()->json(['message' => 'No notification created for Not Started status'], 200);
            } elseif (str_contains($validated['vm_status'], 'Overdue') || 
                    (isset($validated['cm_status']) && str_contains($validated['cm_status'], 'Overdue'))) {
                $notificationStatus = 'Overdue';
                $statusColor = 'bg-red-600 text-red-600';
            } else {
                $days = (int) explode(' ', $validated['vm_status'])[0] ?? 
                        (isset($validated['cm_status']) ? (int) explode(' ', $validated['cm_status'])[0] : 0);
                if ($days >= 8) {
                    $statusColor = 'bg-green-600 text-green-600';
                } elseif ($days >= 6) {
                    $statusColor = 'bg-yellow-600 text-yellow-600';
            ...
            $notification = Notification::updateOrCreate(
                ...
                [
                    'notification_id' => "{$validated['record_id']}-" . now()->toISOString(),
                    'document_no' => $validated['document_no'],
                    'document_type' => $validated['document_type'],
                    'status' => $notificationStatus,
                    'status_color' => $statusColor,
            ...
            \Log::info('Time-based notification updated:', $notification->toArray());

            return response()->json(['message' => 'Notification updated successfully', 'notification' => $notification], 200);
        } catch (\Exception $e) {
            \Log::error('Error updating time notification: ' . $e->getMessage());
